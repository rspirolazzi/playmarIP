...
/**
 * Class Mdl_Service_Item_Amounts
 */
class Mdl_Service_Item_Amounts extends CI_Model
{
    /**
     * item_amount_id
     * item_id
    ...
    public function calculate($item_id)
    {
        ...
        $item = $this->mdl_service_items->get_by_id($item_id);
        ...
        if ($this->db->get('ip_service_item_amounts')->num_rows()) {
            $this->db->where('item_id', $item_id);
            $this->db->update('ip_service_item_amounts', $db_array);
        } else {
            $this->db->insert('ip_service_item_amounts', $db_array);
        }
    }
